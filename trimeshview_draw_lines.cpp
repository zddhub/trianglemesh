...
//    point p2 = triMesh->vertices[v2];
    ...
    if(isSavedOc)
    {
        c_fLines.lines.push_back(Line(p1, p2));
    }
                       ...
                       bool do_test, float fade)
{
    ...
    if(isSavedOc)
    {
        c_fLines.faces.push_back(TriangleMesh::Face(v0, v1, v2));
    }
...
void TriMeshView::draw_silhouette(const vector<float> &ndotv)
{
    ...
    currcolor = vec(0.5, 0.25, 0.0);
    //currcolor = vec(1.0, 0, 0);
    glLineWidth(3);
    glBegin(GL_LINES);
...
bool TriMeshView::saveOccludingFile(const QString &fileName)
{
    if(!triMesh)
        return false;
    ...
    c_fLines.sphere = triMesh->bsphere;


    isSavedOc = true;
    compute_perview(c_ndotv, c_kr);
    draw_isolines(c_ndotv, c_kr, vector<float>(), c_ndotv,
                  false, false, true, 0.0f);

//    compute_perview2(c_ndotv, c_kr);
//    draw_isolines(c_ndotv, c_kr, vector<float>(), c_ndotv,
//                  false, false, true, 0.0f);

    isSavedOc = false;

    return c_fLines.writeLinesToFile(fileName);
}
