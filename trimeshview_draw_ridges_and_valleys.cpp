                        ...
                        float thresh, bool to_center)
{
    ...
    if(isSavedRV)
    {
        ...
        rv_fLines.lines.push_back(Line(p01, p12));
    }
    else
    {
        // Draw the line segment
        if(to_center)
            glColor4f(1.0f, currcolor[1], currcolor[2], k01);
        else
        ...
        if(to_center)
            glColor4f(1.0f, currcolor[1], currcolor[2], k12);
        else
                      ...
                      bool do_bfcull, bool do_test, float thresh)
{
    ...
    if(isSavedRV)
    {
        rv_fLines.faces.push_back(TriangleMesh::Face(v0, v1, v2));
    }
    ...
    if (!z01) {
        ...
        /*
        //先删除一个面上有三条线的情况，这样能保证每个三角形上只有一条线。
...
bool TriMeshView::saveRidgesFile(const QString &fileName)
{
    if(!triMesh)
        return false;

    rv_fLines.clear();
    rv_fLines.sphere = triMesh->bsphere;
    ...
    return rv_fLines.writeLinesToFile(fileName);
}

void TriMeshView::readFeatureLines(const QString &fileName)
{
    fLines.writeLinesToFile(fileName);
}

bool TriMeshView::saveRVMeshFile(const QString &fileName)
{
    return rv_fLines.writeFacesToObj(fileName, triMesh);
}

bool TriMeshView::saveOCMeshFile(const QString &fileName)
{
    return c_fLines.writeFacesToObj(fileName, triMesh);
}
